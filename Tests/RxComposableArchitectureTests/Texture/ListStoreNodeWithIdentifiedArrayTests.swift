...
internal final class ListStoreNodeWithIdentifiedArrayTests: XCTestCase {
    ...
    internal func test_items_shouldHaveNoDuplicate() {
        ...
        let (_, listStore) = setupListStore(state: AppStateWithIdentifiedArray(childs: IdentifiedArray(childs)))
        listStore.didLoad()
    ...
    internal func test_reducer_shouldHaveTriggered() {
        ...
        let (_, ListStore) = setupListStore(state: AppStateWithIdentifiedArray(childs: IdentifiedArray(childs)))
        ListStore.didLoad()
        (ListStore.cellNodes[0].rootNode as! ChildNode).store.send(.increase)
        (ListStore.cellNodes[3].rootNode as! ChildNode).store.send(.increase)
        (ListStore.cellNodes[0].rootNode as! ChildNode).store.send(.increase)
        (ListStore.cellNodes[0].rootNode as! ChildNode).store.send(.increase)
    ...
    internal func test_state_shouldEqual() {
        ...
        let (parentStore, ListStore) = setupListStore(state: AppStateWithIdentifiedArray(childs: IdentifiedArray(childs)))
        ListStore.didLoad()
        (ListStore.cellNodes[0].rootNode as! ChildNode).store.send(.increase)
        (ListStore.cellNodes[3].rootNode as! ChildNode).store.send(.increase)
        (ListStore.cellNodes[0].rootNode as! ChildNode).store.send(.increase)
    ...
    internal func test_changesOnParent_shouldReflectOnChild() {
        ...
        let store = (ListStore.cellNodes[0].rootNode as! ChildNode).store
    ...
    private func setupListStore(state: AppStateWithIdentifiedArray) -> (ParentStore, ListStoreNode<[ChildState], ChildAction>) {
