...
public struct Reducer<State, Action, Environment> {
    ...
    public func pullback<GlobalState, GlobalAction, GlobalEnvironment, StatePath, ActionPath>(
        ...
        action toLoacalAction: ActionPath,
        environment toLocalEnvironment: @escaping (GlobalEnvironment) -> Environment
    ) -> Reducer<GlobalState, GlobalAction, GlobalEnvironment>
    where StatePath: WritablePath, StatePath.Root == GlobalState, StatePath.Value == State,
          ActionPath: WritablePath, ActionPath.Root == GlobalAction, ActionPath.Value == Action {
              return .init { globalState, globalAction, globalEnvironment in
                  guard var localState = toLocalState.extract(from: globalState),
                        let localAction = toLoacalAction.extract(from: globalAction)
                  else { return .none }
                  ...
                  let effect = self
                      ...
                      .map { localAction -> GlobalAction in
                          ...
                          toLoacalAction.set(into: &globalAction, localAction)
                          return globalAction
                      }
    ...
    public func forEach<GlobalState, GlobalAction, GlobalEnvironment, ID>(
        ...
        .init { globalState, globalAction, globalEnvironment in
            ...
            assert(
                ...
                """
                "\(debugCaseOutput(localAction))" was received by a "forEach" reducer at key \(id) \
                when its state contained no element at this key. This is considered an application logic \
                error, and can happen for a few reasons:
