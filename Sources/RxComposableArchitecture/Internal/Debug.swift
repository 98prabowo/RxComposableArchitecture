    ...
    public var debugOutput: String {
        ...
        func updatePath(_ value: Any) {
            ...
            if subjectType.hasPrefix("Delay<") {
                ...
                path.append(
                    "\n.delay(for: \(Double(ns) / Double(NSEC_PER_SEC)), scheduler: \(RxComposableArchitecture.debugOutput(scheduler)))"
                )
            }
            ...
            if subjectType.hasPrefix("ReceiveOn<") {
                let scheduler = mirror.children.first(where: { label, _ in label == "scheduler" })!.value
                path.append("\n.receive(on: \(RxComposableArchitecture.debugOutput(scheduler)))")
            }
        ...
        guard mergeMany.isEmpty else {
            return RxComposableArchitecture
                .debugOutput(mergeMany.filter { !RxComposableArchitecture.debugOutput($0).isEmpty })
        }
        ...
        return """
      \(type(of: self))(\
      \(just.map { "\n\("value: \(RxComposableArchitecture.debugOutput($0))".indent(by: 2))\n" } ?? "")\
