...
public final class ListStoreNode<State, Action>: ASCollectionNode
    ...
    override public var dataSource: ASCollectionDataSource? {
        didSet {
            if oldValue != nil {
    ...
    public convenience init(
        ...
        self.init(
            store: store.scope(
                state: { $0 },
                action: { _, neverAction in neverAction }
            ),
            id: \.id,
            collectionViewLayout: collectionViewLayout,
            content: content
    ...
    public convenience init(
        ...
        self.init(store: store.scope(state: { $0.elements as! State }), id: id, collectionViewLayout: collectionViewLayout, content: content)
    }
    ...
    public convenience init(
        ...
        self.init(store: store, id: \.id, collectionViewLayout: collectionViewLayout, content: content)
    }
    ...
    private func performUpdates(newItems: [State.Element]) {
        ...
        let oldItemsForDiffing = items.map(AnyHashDiffable.init)
        let newItemsForDiffing = newItems.map(AnyHashDiffable.init).removeDuplicates()

        let listDiff = DiffingInterfaceList
            .diffing(oldArray: oldItemsForDiffing, newArray: newItemsForDiffing)
        ...
        let deletes = listDiff.deletes
        let inserts = listDiff.inserts
        let moves = listDiff.moves

        performBatch(animated: shouldAnimateUpdate, updates: {
    ...
    private func diffingCellNode(newItems: [RxComposableArchitecture.AnyHashDiffable], diff: RxComposableArchitecture.DiffingInterfaceList.Result) -> [ListStoreCellNode] {
...
private class ListStoreNodeProxy<State, Action>: NSObject, ASCollectionDataSource
    ...
    fileprivate func collectionNode(_: ASCollectionNode, nodeForItemAt indexPath: IndexPath) -> ASCellNode {
        listStoreNode?.cellNodes[safe: indexPath.section] ?? ASCellNode.createEmptyCell()
    }
}
