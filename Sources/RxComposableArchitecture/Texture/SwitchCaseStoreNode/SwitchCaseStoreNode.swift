...
public class SwitchCaseStoreNode: ASDisplayNode {
    ...
    public struct Matcher<State, Action> {
        ...
        fileprivate init(store: Store<State, Action>, disposeBag: DisposeBag, setNode: @escaping (ASDisplayNode) -> Void) {
            self.store = store
        ...
        public func addMatch<LocalState>(
            ...
            addMatch(state: state, action: { $0 }, createNode: createNode)
        }
        ...
        public func addMatch<LocalState, LocalAction>(
            ...
            store.scope(state: state, action: action).ifLet { [setNode] store in
                let node = createNode(store)
                setNode(node)
    ...
    public init<State, Action>(
        ...
        var matcher = Matcher<State, Action>(store: store, disposeBag: disposeBag) { [weak self] newNode in
            /**
             ...
             */
            if self?.isNodeLoaded == true, SwitchCaseStoreNodeEnvironment.current.isUsingMainThreadExperiment() {
